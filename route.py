...
@app.route('/')
def home():
    members_count = User.query.filter_by(status=True).count()
    return render_template('main.html', members_count=members_count)
    ...
    if login and password:
        ...
        if user and check_password_hash(user.password, password):
            if user.status:
                ...
                flash('Ваш профиль пока не подтвержден')
        else:
...
def register():
    ...
    if request.method == 'POST':
        ...
        if not (login or password or password2 or name or surname or description):
            flash('Пожалуйства, заполните все обязательные поля')
            ...
            new_user = User(login=login, password=hash_pwd, name=name, surname=surname, patronymic=patronymic, description=description)
...
def register_confirm():
    if current_user.login == 'Kostik_number_1':
        accounts_to_confirm_count = User.query.filter(User.status == False).count()
        accounts = User.query.all()
        return render_template('register_confirm.html', accounts=accounts, accounts_to_confirm_count=accounts_to_confirm_count)
    else:
        return redirect('/register')
    
@app.route('/register/confirm/delete', methods=['POST'])
def unconfirn_account():
    account_to_delete = request.form.get('account_to_delete')
    db.session.query(User).filter(User.id == account_to_delete).delete()
    db.session.commit()
    return redirect('/register/confirm')

@app.route('/register/confirm/add', methods=['POST'])
def confirm_account():
    account_to_add = db.session.query(User).filter(User.id == request.form.get('account_to_add')).first()
    account_to_add.status = True
    db.session.commit()
    return redirect('/register/confirm')
